...
DATA_PATH = BASE_DIR / "data" / "bank-1.csv"   # adjust if needed
RANDOM_STATE = 42
...
class ChurnAnalyticsEngine:
    def __init__(self, data, model_name="openai/gpt-oss-20b"):
        ...
        # Prefer Streamlit secrets (works on Streamlit Cloud); fall back to env var
        self.api_key = st.secrets.get("GROQ_API_KEY", os.environ.get("GROQ_API_KEY"))
        self.client = None
        if self.api_key:
            try:
